...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  RichTextCodeMirrorAdapter.prototype.applyOperation = function (operation) {
    ...
    for (var i = 0, l = ops.length; i < l; i++) {
      ...
      if (op.isRetain()) {
        ...
        this.rtcm.insertText(index, op.text, 'RTCMADAPTER');
  ...
  RichTextCodeMirrorAdapter.prototype.invertOperation = function(operation) {
    var pos = 0, cm = this.rtcm.codeMirror;
    var inverse = new TextOperation();
    for(var opIndex = 0; opIndex < operation.ops.length; opIndex++) {
      var op = operation.ops[opIndex];
      if (op.isRetain()) {
